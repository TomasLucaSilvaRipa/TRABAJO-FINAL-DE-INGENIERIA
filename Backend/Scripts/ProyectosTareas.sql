...
BEGIN
    ...
    INSERT INTO dbo.Tarea (IdProyecto,IdEmpleadoAsignado,IdSkillRequerido,IdEstadoTarea,Titulo,Descripcion,Prioridad,Complejidad,FechaInicio,Deadline,SeniorityRequerido,PorcentajeAvance,HorasEstimadas,ArchivosAdjuntosJson,Activo,Bloqueada) VALUES (@IdProyecto,@IdEmpleadoAsignado,@IdSkillRequerido,@IdEstadoTarea,@Titulo,@Descripcion,@Prioridad,@Complejidad,@FechaInicio,@Deadline,@SeniorityRequerido,@PorcentajeAvance,@HorasEstimadas,@ArchivosAdjuntosJson,1,0); SELECT T.* FROM dbo.Tarea T WHERE T.ID=SCOPE_IDENTITY();
END
GO
...
BEGIN
    IF @IdEmpleadoAsignado IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.Empleado E INNER JOIN dbo.Usuario U ON U.ID=E.IdUsuario WHERE E.ID=@IdEmpleadoAsignado AND U.IdAgencia=@IdAgencia)
        SELECT @IdEmpleadoAsignado=E.ID FROM dbo.Empleado E INNER JOIN dbo.Usuario U ON U.ID=E.IdUsuario WHERE E.IdUsuario=@IdEmpleadoAsignado AND U.IdAgencia=@IdAgencia;
    IF @IdEmpleadoAsignado IS NOT NULL AND NOT EXISTS (SELECT 1 FROM dbo.Empleado WHERE ID=@IdEmpleadoAsignado) THROW 50002, 'El empleado asignado no pertenece a la agencia.', 1;
    UPDATE T SET IdProyecto=@IdProyecto,IdEmpleadoAsignado=@IdEmpleadoAsignado,IdSkillRequerido=@IdSkillRequerido,IdEstadoTarea=@IdEstadoTarea,Titulo=@Titulo,Descripcion=@Descripcion,Prioridad=@Prioridad,Complejidad=@Complejidad,FechaInicio=@FechaInicio,Deadline=@Deadline,SeniorityRequerido=@SeniorityRequerido,PorcentajeAvance=@PorcentajeAvance,HorasEstimadas=@HorasEstimadas,ArchivosAdjuntosJson=@ArchivosAdjuntosJson FROM dbo.Tarea T INNER JOIN dbo.Proyecto P ON P.ID=T.IdProyecto WHERE T.ID=@ID AND P.IdAgencia=@IdAgencia; SELECT T.* FROM dbo.Tarea T INNER JOIN dbo.Proyecto P ON P.ID=T.IdProyecto WHERE T.ID=@ID AND P.IdAgencia=@IdAgencia;
END
GO
